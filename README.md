...
Create a local `.env` from `.env.example` and set the client-safe browser values:
...
If your Supabase project values change, update `.env` with:
...
- `vite.config.ts` injects CSP, canonical URLs, Open Graph metadata, structured data, and an autogenerated sitemap during build.
- `public/sw.js` provides offline caching for the app shell and static assets after the first successful load.
- `.github/workflows/ci.yml` runs lint, tests, typecheck, build, and an npm audit check on pushes and pull requests.
- `.github/workflows/deploy.yml` now runs the same quality gates before publishing GitHub Pages.
