          ...
          <div className="flex items-center gap-1">
            <div className="flex items-center gap-1 rounded bg-slate-50 p-0.5 dark:bg-slate-800">
              <input
                ...
                className="w-32 bg-transparent px-1 text-[0.7rem] font-black uppercase outline-none"
                defaultValue="UNTITLED_PROJECT"
              />
              <span className="text-[0.7rem] font-bold text-slate-400">.T2S</span>
            </div>
            ...
            <div className="flex h-6 items-center rounded border border-slate-200 bg-white px-1 dark:border-slate-700 dark:bg-slate-800">
              <select id="downloadFormat" aria-label="Export format" className="bg-transparent text-[0.65rem] font-bold outline-none">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded bg-emerald-500 px-2 py-1 text-[0.7rem] font-black uppercase text-white shadow-sm hover:bg-emerald-600 transition-all active:scale-95"
            >
              <Globe size={12} /> Post
            </button>
          </div>
          <div className="flex items-center gap-3">
            <span id="cloudAuthState" className="text-[0.7rem] font-black uppercase tracking-widest text-slate-400">Checking session...</span>
          </div>
        </div>
        ...
        <div className="border-b border-black/5 bg-slate-50/70 px-2 py-1.5 dark:border-slate-800 dark:bg-[#11161d]">
          <div className="space-y-2">
            <div
              ...
              className={`flex items-center gap-2 rounded-md px-3 py-2 text-[0.68rem] font-bold ${workspaceStatusStyles[workspaceStatus.severity]}`}
            >
              {isBootingWorkspace ? (
            ...
            {workspaceTroubleshooting.length > 0 && (
              <div className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-[0.7rem] leading-relaxed text-rose-900 dark:border-rose-900/30 dark:bg-rose-900/10 dark:text-rose-100">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    ...
                    className="rounded-md bg-rose-600 px-3 py-1.5 text-[0.65rem] font-black uppercase tracking-widest text-white hover:bg-rose-700"
                  >
                    Retry boot
                  </button>
                  <button
                    ...
                    className="rounded-md border border-rose-200 bg-white px-3 py-1.5 text-[0.65rem] font-black uppercase tracking-widest text-rose-800 hover:bg-rose-100 dark:border-rose-900/30 dark:bg-transparent dark:text-rose-100 dark:hover:bg-rose-900/20"
                  >
                    Reload page
                  </button>
                </div>
        ...
        <div className="flex flex-1 overflow-hidden p-0.5 gap-0.5 select-none" style={{ userSelect: isResizingSidebar || isResizingPreview ? 'none' : 'auto' }}>
          ...
          <div className="flex w-16 flex-col items-center gap-3 border-r border-black/5 bg-white py-4 dark:border-slate-800 dark:bg-[#161b22] shrink-0">
            {categories.map(cat => (
              ...
              >
                ...
                <span className="text-[0.65rem] font-black uppercase tracking-tight text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{cat.label}</span>
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-col border-r border-black/5 bg-white dark:border-slate-800 dark:bg-[#161b22] shrink-0" style={{ width: sidebarWidth }}>
            <div className="flex h-8 items-center justify-between border-b border-slate-50 px-3 dark:border-slate-800">
              <span className="text-[0.7rem] font-black uppercase tracking-widest text-slate-400">Registry</span>
              <a href="reference.html" target="_blank" rel="noreferrer" aria-label="Open reference in new tab" className="text-blue-600 hover:opacity-80 transition-opacity"><ScanSearch size={12} /></a>
            </div>
          ...
          <div
            ...
            className="w-1 cursor-col-resize shrink-0 transition-colors hover:bg-blue-500/30"
          />
          ...
          <div className="flex flex-1 flex-col border-r border-black/5 bg-white dark:border-slate-800 dark:bg-[#0d1117]">
            ...
            <div className="h-20 border-t border-slate-50 bg-[#f9f9f9] p-2 dark:border-slate-800 dark:bg-[#161b22]">
              <div className="mb-1 flex items-center gap-1 text-[0.7rem] font-black uppercase tracking-widest text-slate-400">
          ...
          <div className="flex flex-col gap-0.5 overflow-hidden shrink-0" style={{ width: previewWidth }}>
            ...
            <div className="flex flex-col border border-black/5 bg-white p-1.5 dark:border-slate-800 dark:bg-[#161b22]">
              <div className="mb-1.5 flex items-center justify-between px-1">
                ...
                <div className="flex gap-1">
                  <button type="button" id="previewRunBtn" aria-label="Run preview" className="flex h-6 w-6 items-center justify-center rounded-full bg-[#2da44e] text-white shadow hover:opacity-90 active:scale-90 transition-all">
                    <Play size={12} fill="currentColor" />
                  </button>
                  <button type="button" id="previewStopBtn" aria-label="Stop preview" className="flex h-6 w-6 items-center justify-center rounded-full bg-rose-500 text-white shadow hover:opacity-90 active:scale-90 transition-all">
              ...
              <div className="relative aspect-[4/3] overflow-hidden rounded border border-slate-200 bg-black shadow-inner">
                ...
                <div id="previewOverlay" className="absolute inset-0 flex items-center justify-center bg-black/60 font-black text-[0.7rem] uppercase tracking-[0.3em] text-white/50">
                  Preview not started
                </div>
              </div>
              <div className="mt-1 flex justify-between px-1 text-[0.7rem] font-bold uppercase tracking-tighter text-slate-400">
            ...
            <div className="flex-1 flex flex-col border border-black/5 bg-white p-3 dark:border-slate-800 dark:bg-[#161b22] overflow-y-auto custom-scrollbar">
              <div className="mb-3 flex items-center gap-2 border-b border-slate-50 pb-2 dark:border-slate-800 shrink-0">
                ...
                <span className="text-[0.7rem] font-black uppercase tracking-[0.2em] text-slate-400">Cloud projects</span>
              </div>
              <div className="space-y-4 pb-2">
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded border border-slate-200 bg-white px-2 py-1 text-[0.6rem] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
                    >
                      <RefreshCw size={10} />
                      Refresh
                    </button>
                  </div>
                  <select id="cloudProjectsSelect" aria-label="Saved cloud projects" className="w-full rounded border border-slate-200 bg-slate-50 px-2 py-1.5 text-[0.7rem] font-bold dark:border-slate-700 dark:bg-slate-800 outline-none focus:border-blue-400">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <input id="shareLinkOutput" aria-label="Share link" readOnly className="w-full rounded border border-slate-200 bg-slate-50 px-2 py-1.5 text-[0.65rem] font-mono dark:border-slate-700 dark:bg-slate-800" placeholder="NULL" />
                    <button type="button" id="copyShareLinkBtn" aria-label="Copy share link" className="w-full rounded bg-[#4d97ff] py-1.5 text-[0.7rem] font-black uppercase text-white shadow-sm hover:opacity-90 active:scale-95 transition-all">Copy Deployment Link</button>
                  </div>
                </div>
                <div id="sharedProjectNotice" role="status" aria-live="polite" className="rounded border border-blue-100 bg-blue-50/50 p-2 text-[0.65rem] font-bold text-blue-600 dark:bg-blue-900/10 dark:border-blue-900/20" hidden />
              </div>
